...
function IotPanel() {
  return <Card className="p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><h3 className="text-sm sm:text-base font-bold">Gerçek Zamanlı IoT Verileri</h3><button className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold">Tüm Cihazlar</button></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{iotCards.map(([name, value, status, Icon, color, kind]) => <div key={name} className="rounded-xl border border-slate-200 p-4"><div className="flex items-center justify-between text-xs text-slate-500"><span>{name}</span><Icon style={{ color }} /></div><p className="mt-3 text-xl font-bold">{value}</p><div className="mt-2 flex items-center justify-between"><span className={`text-xs font-bold ${iotStatusColor(status, kind)}`}>{status}</span><div className="h-8 w-20"><MiniLine color={color} dataKey="a" /></div></div></div>)}</div></Card>
}
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-12">
        ...
        <Card className="min-w-0 p-4 sm:p-5 xl:col-span-4"><div className="mb-4 flex items-center justify-between"><h3 className="text-sm sm:text-base font-bold">ESG Performansı</h3><button className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold">2024</button></div><div className="h-64"><ResponsiveContainer><RadarChart data={radar}><PolarGrid /><PolarAngleAxis dataKey="subject" /><Radar dataKey="score" stroke="#0f766e" fill="#14b8a6" fillOpacity={0.18} /></RadarChart></ResponsiveContainer></div><p className="-mt-10 text-center text-3xl font-bold">78</p><p className="text-center text-xs text-slate-500">ESG Skoru</p></Card>
      </div>
