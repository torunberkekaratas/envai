...
export default function ModulePage({ title, variant = 'standard' }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3 sm:space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-12">
        <div className="min-w-0 xl:col-span-8"><DataTable title="Operasyon Kayıtları" rows={config.table} /></div>
        <ShellCard className="min-w-0 p-4 sm:p-5 xl:col-span-4"><h3 className="mb-4 text-sm sm:text-base font-bold">Öncelikli Aksiyonlar</h3><div className="space-y-3">{config.workflows.slice(0, 5).map((item, index) => <div key={item} className="flex items-start gap-3 rounded-xl bg-slate-50 p-3 dark:bg-white/[0.04]"><FiCheckCircle className="mt-1" style={{ color: config.accent }} /><div><p className="text-sm font-bold">{item}</p><p className="text-xs text-slate-500">Sahip: {['Operasyon', 'ESG', 'Finans', 'Veri Ekibi', 'Tedarik'][index]}</p></div></div>)}</div></ShellCard>
      </div>
    </motion.div>
  )
}
